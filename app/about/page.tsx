...
const LINKS = [
  { label: "Aether Theme",       desc: "Our Shopify theme for independent brands.",       href: "/aether",     external: false },
  { label: "Inertia Dashboard",  desc: "Client ops, in one place. In progress.",           href: "/roadmap",    external: false },
  { label: "Work with us",       desc: "Custom Shopify builds and brand projects.",         href: "/contact",    external: false },
  { label: "Careers",            desc: "Join the team.",                                   href: "/careers",    external: false },
];
...
export default function AboutPage() {
  ...
  return (
    <main className="mx-3 sm:mx-auto w-auto sm:w-full max-w-[80rem] min-h-screen flex flex-col pb-16 sm:pb-20">
      ...
      {/* Hero — split layout */}
      <section className="flex flex-col sm:flex-row gap-12 sm:gap-20 px-3 pt-16 sm:pt-24 pb-16 rise">
        ...
        <div className="flex flex-col gap-7 flex-1">
          ...
          <h1 className="text-[clamp(2.6rem,4.5vw,3.5rem)] font-normal tracking-[-0.04em] leading-[1.02] text-[rgb(var(--fg))]">
            Convert wasted time
            <br />
            <span style={{ color: "rgb(var(--muted))", opacity: 0.5 }}>into momentum</span>
          </h1>
        ...
        {/* Right — linked items */}
        <div className="flex flex-col w-full sm:w-[600px] shrink-0 self-start sm:pt-12">
          {LINKS.map((l, i) => (
            <Link
              key={l.label}
              href={l.href}
              className="group flex items-start justify-between gap-4 py-4 border-t border-[rgb(var(--line))] hover:bg-[rgb(var(--fg)/0.02)] transition-colors"
              style={{ borderBottom: i === LINKS.length - 1 ? "1px solid rgb(var(--line))" : undefined }}
            >
              ...
              <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" className="w-3.5 h-3.5 shrink-0 mt-1 text-[rgb(var(--muted))] opacity-30 group-hover:opacity-60 transition-opacity" aria-hidden="true">
      ...
      {/* Pull quote */}
      <section className="px-3 py-20 sm:py-28 rise">
        <div className="max-w-3xl mx-auto text-center">
          <p className="text-[clamp(1.4rem,2.5vw,2rem)] font-normal tracking-[-0.03em] leading-[1.5] text-[rgb(var(--fg))]">
            <span className="text-[1.4em] leading-none text-[rgb(var(--muted))] select-none" style={{ opacity: 0.2, fontFamily: "Georgia, serif" }}>&ldquo;</span>
            Most people spend their days inside systems that were never designed to work for them. We think that&apos;s fixable. Not by doing things faster. By removing the need to do them at all.
            <span className="text-[1.4em] leading-none text-[rgb(var(--muted))] select-none" style={{ opacity: 0.2, fontFamily: "Georgia, serif" }}>&rdquo;</span>
          </p>
        </div>
      </section>
      ...
      <section className="px-3 pt-16 sm:pt-24 pb-16 rise">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-[rgb(var(--line))]">
          {PRINCIPLES.map((p, i) => (
            <div key={p.title} className="flex gap-5 p-8 bg-[rgb(var(--bg))] hover:bg-[rgb(var(--surface))] transition-colors" style={{ ["--rise-delay" as any]: `${i * 60}ms` }}>
              <span className="text-[clamp(2rem,4vw,3rem)] font-normal tracking-[-0.05em] leading-none text-[rgb(var(--muted))] shrink-0 select-none" style={{ opacity: 0.1 }}>
                {String(i + 1).padStart(2, "0")}
              </span>
              <div className="flex flex-col gap-2.5 pt-1">
                <h3 className="text-[15px] font-medium tracking-tight text-[rgb(var(--fg))] leading-snug">{p.title}</h3>
                <p className="text-[14px] leading-relaxed tracking-tight text-[rgb(var(--muted))]">{p.body}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="px-3 py-16 sm:py-24 rise">
        <div className="rounded-xl border border-[rgb(var(--line))] bg-[rgb(var(--surface))] px-8 sm:px-16 py-14 sm:py-20 flex flex-col items-center text-center gap-6 max-w-3xl mx-auto">
